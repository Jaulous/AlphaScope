...
21. The homepage now server-renders the latest shaped snapshot so the first HTML response already contains the current dashboard state instead of forcing the user through a blank loading pass before hydration completes.
22. The hydrated client still refreshes `/api/dashboard/latest` in the background on an interval, but refresh failures degrade to a non-blocking stale-data warning instead of wiping the visible snapshot with a hard error state.
23. The frontend-local `/api/dashboard/latest` proxy no longer forwards the backend payload unchanged; it trims the response to the dashboard-visible indicator/theme/stock slices, preserves separate total counts, and reduces `latest_run` to the runtime fields the homepage actually renders.
24. The frontend-local dashboard proxy now advertises short HTTP caching (`max-age` plus `stale-while-revalidate`) so repeated page opens during the same monitoring window can reuse the latest shaped snapshot instead of re-downloading the full response every time.
25. On this machine, the local web-to-backend path must prefer `127.0.0.1` over `localhost` for the backend origin because Node/Next dev requests to `localhost:8000` can stall for many seconds even when the backend itself is healthy.
